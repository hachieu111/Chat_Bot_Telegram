...
// PORT: Render sẽ tự gán qua process.env.PORT
...
if (!token || token.includes('YOUR_TELEGRAM_BOT_TOKEN_HERE')) {
    ...
    console.error('   Please set TELEGRAM_TOKEN in environment variables');
    process.exit(1);
}

// Mask token cho logging an toàn
const maskedToken = token.substring(0, 10) + '...' + token.substring(token.length - 5);

// Khởi tạo bot - QUAN TRỌNG: CHỈ DÙNG WEBHOOK, KHÔNG POLLING
const bot = new TelegramBot(token, {
    polling: false,  // TẮT POLLING ĐỂ TRÁNH LỖI 502
    onlyFirstMatch: true,
    request: {
        timeout: 30000,
        agentOptions: {
...
console.log('🤖 Bot initialized (webhook mode only)');
...
// 1. Webhook endpoint - KHÔNG CHỨA TOKEN TRONG URL
app.post('/webhook', (req, res) => {
    const updateId = req.body.update_id;
    console.log(`📩 Webhook received - Update ID: ${updateId}`);
...
app.get('/admin/dashboard', (req, res) => {
    ...
    res.send(`
        ...
        <body>
            ...
            <div class="stats">
                <h3>📊 Statistics</h3>
                <p>Total Users: ${database.getUserCount()}</p>
                <p>Active Today: ${database.getActiveUsersCount()}</p>
                <p>AI Usage: ${apiMonitor.getAICount()} calls</p>
            </div>
        </body>
        </html>
    `);
...
async function setupWebhook() {
    try {
        // Chỉ setup webhook khi có external URL (production)
        const webhookUrl = process.env.NGROK_URL 
                    ? `${process.env.NGROK_URL}/webhook`
                    : (process.env.RENDER_EXTERNAL_URL 
                        ? `${process.env.RENDER_EXTERNAL_URL}/webhook`
                        : null);
        
        if (!webhookUrl) {
            console.warn('⚠️ Running in local mode (no webhook URL set)');
            console.warn('   Set RENDER_EXTERNAL_URL for production webhook');
            return;
        }
        ...
        console.log(`✅ Webhook set to: ${webhookUrl.replace(/:[^/]+/, ':*****')}`);
...
app.listen(PORT, '0.0.0.0', async () => {
    ...
    console.log(`🤖 AI Enabled: ${config.ENABLE_AI ? 'Yes' : 'No'}`);
    ...
    if (process.env.RENDER) {
        console.log('🏗️  Running on Render.com');
        if (process.env.RENDER_EXTERNAL_URL) {
            console.log(`🌐 External URL: ${process.env.RENDER_EXTERNAL_URL}`);
            console.log(`📡 Webhook: ${process.env.RENDER_EXTERNAL_URL}/webhook`);
            console.log(`📊 Stats: ${process.env.RENDER_EXTERNAL_URL}/api/stats`);
        }
    }
});
...
bot.on('polling_error', (error) => {
    ...
    // Tắt polling nếu vô tình được bật
...
console.log(`📡 Webhook mode: ${process.env.RENDER_EXTERNAL_URL ? 'Active' : 'Local'}`);
console.log(`🤖 AI: ${config.ENABLE_AI ? 'Enabled' : 'Disabled'} (${config.AI_MODE} mode)`);