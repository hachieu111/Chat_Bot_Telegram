...
function findSkillIdByApproximateName(text) {
  ...
  // Chuyển về không dấu và viết thường
  const normalize = (str) => {
    if (!str) return '';
  ...
  // Nếu text quá ngắn, không tìm kiếm
  if (normalizedText.length < 3) return null;
  
  // Tìm kỹ năng có tên khớp
  for (const skill of skills) {
    const normalizedSkillName = normalize(skill.name);
    
    // Kiểm tra khớp toàn bộ hoặc một phần
    ...
    // Kiểm tra khớp không dấu với tên ngắn
...
async function handlePostback(event) {
  ...
  if (payload.startsWith('VIEW_SKILL_')) {
    ...
    console.log(`🔍 Xử lý VIEW_SKILL: ${skillId}`);
    await showSkillInfo(senderId, userId, skillId);
    return;
  }
  ...
  if (payload.startsWith('START_SKILL_')) {
    ...
    console.log(`🔍 Xử lý START_SKILL: ${skillId}`);
  ...
  if (payload.startsWith('SKILL_') && payload.includes('_STEP_')) {
    ...
    if (match) {
      ...
      console.log(`🔍 Xử lý SKILL_STEP: ${skillId}, bước ${stepIndex}`);
      const skill = skills.find(s => s.id === skillId);
      if (skill) {
  ...
  if (payload.startsWith('ANSWER_')) {
    ...
    if (match) {
      ...
      console.log(`🔍 Xử lý ANSWER: ${skillId}, bước ${stepIndex}, đáp án ${answerId}`);
      const skill = skills.find(s => s.id === skillId);
      const stepNum = parseInt(stepIndex);
  ...
  if (payload.startsWith('FREE_RESPONSE_')) {
    ...
    if (match) {
      const [, skillId, stepIndex] = match;
      console.log(`🔍 Xử lý FREE_RESPONSE: ${skillId}, bước ${stepIndex}`);
      database.updateUser(userId, { 
  ...
  if (payload.startsWith('RETRY_')) {
    ...
    if (match) {
      const [, skillId, stepIndex] = match;
      console.log(`🔍 Xử lý RETRY: ${skillId}, bước ${stepIndex}`);
      const skill = skills.find(s => s.id === skillId);
  ...
  if (payload.startsWith('COMPLETE_SKILL_')) {
    ...
    console.log(`🔍 Xử lý COMPLETE_SKILL: ${skillId}`);
    const skill = skills.find(s => s.id === skillId);
