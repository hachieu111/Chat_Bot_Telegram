...
console.log('🚀 Khởi động Messenger Bot...');
...
async function handleMessage(event) {
  ...
  if (messageText.match(/^(bắt đầu|start|hello|chào|hi|xin chào)$/i)) {
    const welcomeText = `👋 *Chào mừng đến với SEED Career Skills!*\n\n` +
      ...
      `💡 *Bắt đầu bằng cách chọn một tùy chọn bên dưới:*`;
    
    await sendMainMenu(senderId, userId);
    
  } else if (messageText.match(/^(menu|mục lục|chức năng)$/i)) {
    await sendMainMenu(senderId, userId);
    
  } else if (messageText.match(/^(kỹ năng|skills|học|bài học)$/i)) {
    await sendSkillMenu(senderId, userId);
    
  } else if (messageText.match(/^(tiến độ|progress|hoàn thành)$/i)) {
    const user = database.getUser(userId);
    const completed = user.completedSkills?.length || 0;
    const total = skills.length;
    const progress = Math.round((completed / total) * 100);
    ...
    // Xử lý AI response nếu đang trong chế độ free response
    const user = database.getUser(userId);
...
async function handlePostback(event) {
  ...
  switch (payload) {
    ...
    default:
      ...
      } else if (payload.startsWith('SKILL_') && payload.includes('_STEP_')) {
      ...
      } else if (payload.startsWith('ANSWER_')) {
      ...
      } else if (payload.startsWith('FREE_RESPONSE_')) {
      ...
      } else if (payload.startsWith('HINT_')) {
        const match = payload.match(/HINT_(.+)_(\d+)/);
        if (match) {
          const [, skillId, stepIndex] = match;
          const skill = skills.find(s => s.id === skillId);
          ...
          if (step && config.ENABLE_AI) {
            try {
              ...
              await sendQuickReplies(senderId, `💡 *GỢI Ý:*\n\n${hint}`, [
                { title: '🔄 Thử lại', payload: `SKILL_${skillId}_STEP_${stepIndex}` }
              ]);
            } catch (error) {
      ...
      } else if (payload.startsWith('RETRY_')) {
...
app.listen(PORT, async () => {
  console.log(`🚀 Messenger Bot đang chạy trên port ${PORT}`);
  console.log(`🌐 Webhook URL: http://localhost:${PORT}${config.WEBHOOK_PATH}`);
  console.log(`🤖 AI Mode: ${config.AI_MODE}`);
  
  // Setup Messenger Profile
  if (PAGE_ACCESS_TOKEN && config.NODE_ENV === 'production') {
    await setupMessengerProfile();
  }
});

// Export cho testing
module.exports = { app, sendMessage, sendQuickReplies };